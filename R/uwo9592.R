...
#' Make Data for MICE Posterior Predictive Distributions
#' 
#' Makes data that will permit the generation of the posterior predictive
#' distribution using `mice()` from the `mice` package.  The function replicates
#' the original input data `k` times, each time setting one of the variables 
#' to completely missing such that all its values will be imputed from the imputation 
#' model.  See the help for `process_post()` for managing the data after imputation. 
#' @param data The original data that were used in the `mice()` call. 
#' @param ... Currently not implemented. 
#' @export
make_post_data <- function(data, ...){
  has_missing <- lapply(data, \(x)any(is.na(x)))
  orig <- data
  orig$post_var <- "orig"
  out <- orig
  for(i in seq_along(has_missing)){
    if(has_missing[[i]]){
      tmp <- data
      tmp[[names(has_missing)[i]]] <- NA
      tmp$post_var <- names(has_missing)[i]
...
#' Process Posterior Predictive MICE Distributions
#' 
#' Processes the posterior predictive distributions generated by `mice()` 
#' using the output from `make_post_dat()`.  
#' @param imps A `mice` object that was estimated on data that were generated
#' using `make_post_data()`.  
#' @param data The data used in the original call to `mice`. 
#' @param ... Currently not implemented. 
#' @importFrom mice complete
process_post <- function(imps, data, ...){
  comps <- lapply(1:imps$m, \(i)complete(imps, i))
  orig <- lapply(comps, \(x)x[which(x$post_var == "orig"), ])
  post <- lapply(1:imps$m, \(i)data)
  impvars <- setdiff(unique(comps[[1]]$post_var), "orig")
  for(i in seq_along(post)){
    for(v in impvars){
      post[[i]][[v]] <- comps[[i]][[v]][which(comps[[i]]$post_var == v)]
    }
  }
